    ...
    <div class="profile-con__inner--right">
      <section class="profile-sec">
        <div class="profile-sec__inner">
          ...
          <ul class="about-me">
            <li class="about-me__list">{{ $moment(profileData.birth_day).format('YYYY年MM月DD日') }}</li>
          ...
          >
            <dt
              class="career__term"
            >{{ $moment(career.date_from).format('YYYY年MM月DD日') + '〜' + $moment(career.date_to).format('YYYY年MM月DD日')}}</dt>
            <dd class="career__desc">{{ career.content }}</dd>
          </dl>
          <hr class="under-line" />
          <ul>
            <li
